...
library(autoencoder)
load("data/categoryEachDay.RData")

naCount <- apply(day_time_category_df, 1, function(x) length(which(is.na(x))))
sum(naCount==0)
noNA_category_df <- day_time_category_df[which(naCount==0),]
